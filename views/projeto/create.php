...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // --- VALIDAÇÃO (REMOVIDO DATA_INICIO) ---
        ...
        // --- ALTERADO: Tratamento da Data Início para NULL se estiver vazia ---
        ...
        $params = [
            ...
            ':data_inicio' => $data_inicio, // Usando a variável tratada
            ':status' => $_POST['status'],
            ':data_fim' => $data_fim,
        ...
        if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] === UPLOAD_ERR_OK) {
            ...
            if (array_key_exists($mime, $permitidos)) {
                ...
                if (ImageHandler::resizeAndSave($_FILES['imagem'], $diretorio_upload, $novo_nome, 1024)) {
                    ...
                    // Fallback: Se o resize falhar (servidor sem memória, etc), tenta o move simples
                    move_uploaded_file($_FILES['imagem']['tmp_name'], $diretorio_upload . $novo_nome);
                    // (Opcional: registrar erro de log)
                }
            }
        }
        ...
        if ($db->inTransaction())
            $db->rollBack();
        $erro = $e->getMessage();
        $dados_form = $_POST; // Para repopular o formulário em caso de erro
    }
}
?>

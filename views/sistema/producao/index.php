    ...
    <div class="page-header">
        <div class="container">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="mb-2"><i class="bi bi-journal-text me-2"></i> Produções Acadêmicas</h1>
                    <p class="mb-0">
    ...
    <script>
        ...
        tableViewBtn.addEventListener('click', function () {
            ...
            if (!$.fn.DataTable.isDataTable('#producoesTable')) {
                $('#producoesTable').DataTable({ language: { url: '//cdn.datatables.net/plug-ins/1.13.6/i18n/pt-BR.json' }, pageLength: 10, responsive: true, columnDefs: [{ orderable: false, targets: [5] }], order: [[3, 'desc']] });
            }
        });
        ...
        function filterProducoes() {
            ...
            document.querySelectorAll('.producao-item').forEach(item => {
                const titulo = item.getAttribute('data-titulo') || item.innerText.toLowerCase();
                const tipo = item.getAttribute('data-tipo');
                const matchesSearch = titulo.includes(searchTerm);
                const matchesTipo = tipoValue === '' || tipo === tipoValue;
                item.style.display = (matchesSearch && matchesTipo) ? '' : 'none';
            });
        }
        ...
        $(document).ready(function () {
            $('.filter-select').selectpicker({ style: 'btn-light', size: 4 });
